namespace MCPCursorExtension {
  export namespace Tests {
    describe('Performance Benchmarks', () => {
      let graph: MCPCursorExtension.AdvancedKnowledgeGraph;

      beforeEach(() => {
        graph = new MCPCursorExtension.AdvancedKnowledgeGraph();
      });
    ...
    function createTestNode(id: string, coordinates: { x: number; y: number }): MCPCursorExtension.EnhancedGraphNode {
      return {
        id,
        type: 'temporal',
        weight: 1.0,
    ...
    function createTestNodes(): Map<string, MCPCursorExtension.EnhancedGraphNode> {
      const nodes = new Map<string, MCPCursorExtension.EnhancedGraphNode>();
